...
class PDFTelefonosImpresoras extends Rotation
{
 ...
 public function encabezadoTabla(){
    ...
    private function details()
    {
        ...
        $this->SetFont('Arial', '', $this->txtContenidoTam);
        $this->CellFitScale(0.55 * $this->WidthTotal, 0.5, count($this->data['telefonos']), '', 1, 'L');
    ...
    function items()
    {
        ...
        $num=1;
        $numItems = count($this->data['telefonos']);
        $a=0;
        $this->encabezadoTabla();


        foreach ($this->data['telefonos'] as $key => $item) {
            ...
            if ($key + 1 == $numItems ) {
                ...
                $this->Row(array(
                        ...
                        $item['modelo'],
                        $item['impresora']['mac'],
                        $item['impresora']['marca'],
                        $item['impresora']['modelo']
                    )
                );
                $this->encola = "";
        }
